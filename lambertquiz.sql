...
OR REPLACE FUNCTION get_users_quizzes_10_stats () RETURNS TABLE (
  ...
  betterScore BIGINT,
  quizzesCompletitionPercentage NUMERIC
) AS $$
BEGIN
RETURN QUERY
  SELECT DISTINCT
  ...
  ROUND(
    CAST(
      (
        CAST(COUNT(DISTINCT Quizzes.quiz_id) AS FLOAT) / 3
      ) AS NUMERIC
    ),
    2
  ) AS quizzesCompletitionPercentage
FROM
  Progresses
...
CREATE
OR REPLACE FUNCTION get_last_10_days_quizzes (IN _user_id INTEGER) RETURNS TABLE (day DATE, totalQuizzes BIGINT) AS $$
BEGIN
    RETURN QUERY
DROP VIEW IF EXISTS
  last_10_days;
CREATE OR REPLACE VIEW
  last_10_days AS
SELECT
  (CURRENT_DATE - (generate_series(0, 9)))
FROM
  generate_series(0, 9)
LIMIT
  10;
...
FROM
  ...
  JOIN Quizzes ON Progresses.quiz = Quizzes.quiz_id
  JOIN Users ON Progresses._user = Users.user_id
  CROSS JOIN last_10_days
WHERE
  Users.user_id = _user_id
GROUP BY
  last_10_days.day,
  Progresses.quiz_finished_at
HAVING
  EXTRACT(
    DAY
    FROM
...
CREATE
OR REPLACE FUNCTION get_user_lambertquiz_days (IN _user_id INTEGER) RETURNS TABLE (username VARCHAR, days INTEGER, subscription DATE) AS $$
BEGIN
    RETURN QUERY
    SELECT DISTINCT
  Users.username,
  EXTRACT(DAY FROM (NOW() - Users.inserted_at)) + 1 AS days,
  TO_CHAR(Users.inserted_at, 'DD/MM/YYYY HH24:MI') AS subscription
FROM
  Users
WHERE
  Users.user_id = _user_id;
...
CREATE
OR REPLACE FUNCTION get_user_pref_category (IN _user_id INTEGER) RETURNS TABLE (category VARCHAR, count INTEGER) AS $$
BEGIN
  RETURN QUERY
  DROP VIEW IF EXISTS user2categorycount;

CREATE OR REPLACE VIEW categorycount AS
SELECT DISTINCT
  Quizzes.category,
  COUNT(DISTINCT Quizzes.quiz_id) AS count
FROM
  Progresses
  JOIN Users ON Progresses._user = Users.user_id
  JOIN Quizzes ON Progresses.quiz = Quizzes.quiz_id 
